...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
  ...
  //  int ind = insertion_idx (subroot->elements,key);
  //  if (ind
 
    //TODO
    int  first_larger_idx = insertion_idx(subroot->elements, key);
    if (!(first_larger_idx>=subroot->elements.size() or first_larger_idx<0) )
    {
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    // parent->children[mid_child_idx]= new_left;
   
    // (parent->children).insert(mid_child_itr, new_right);
/*
    for (typename vector<BTreeNode*>::iterator it = mid_child_itr ; it <child->children.end() ; it++)
    {
        new_right->children.push_back(*it);
 
    }
    for (int i = 0 ; i<mid_child_idx ; i++)
    {
        new_left->children.push_back(child->children[i]);
 
    }
    for (typename vector<DataPair>::iterator it = mid_elem_itr ; it <child->elements.end() ; it++)
    {
        new_right->elements.push_back(*it);
 
    }
    for (int i=0 ; i<mid_elem_idx ;i++)
    {
 
        new_left->elements.push_back(child->elements[i]);
    }
    */
   //parent->children[mid_child_idx]= new_left;
    //(parent->children).insert(mid_child_iter,mid_child_idx+1,new_right);
...
/*
    ...
    if (first_larger_idx<subroot->elements.size() and first_larger_idx>0)
    {
        subroot->elements.insert(subroot->elements.begin() + first_larger_idx,pair);
        if(subroot->elements.size()>order)
            split_child(subroot,subroot->children.size()/2 +1);
...
*/
    ...
    {
        if (first_larger_idx>=0 and first_larger_idx<subroot->elements.size())
        {
