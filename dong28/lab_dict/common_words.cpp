...
vector< string > CommonWords::get_common_words(unsigned int n) const
{
    /* Your code goes here! */
        ...
        for(auto i = common.begin(); i != common.end(); i++){
                if(i->second == file_word_maps.size()){         // this word(i->second) appears in all files           
                        int count = 0;                 
                        for(size_t j = 0; j < file_word_maps.size(); j++){  //in every file this word                                          
                                        if((file_word_maps[j].find(i->first))->second >= n){          // more than n times
                                        count++;
                                }
                        }
